@forelse ($watchlists as $watchlist)
    <div class="watchlist-card cust-row">
        <a href="{{ route('show-movie', $watchlist->id) }}" class="watchlist-card__poster row-item"
            style="background-image : url('{{ asset('storage/movies/thumbnail/' . $watchlist->image_url) }}');background-size: contain;background-repeat: no-repeat;background-position: left;min-height: 150px;">
        </a>
        ...
        <div class="actions row-item">
            <button type="button" class="btn btn-primary status-btn" data-bs-toggle="modal"
                data-bs-target="#movie-{{ $watchlist->id }}">
                &#8943;
            </button>

            <div class="modal fade" id="movie-{{ $watchlist->id }}" tabindex="-1"
