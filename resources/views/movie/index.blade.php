            ...
            <div class="carousel-inner">
                @foreach ($randomMovies as $randMovie)
                    <div class="carousel-item carousel-header{{ $randomMovies->first() == $randMovie ? ' active' : '' }}">
                        <img src="{{ asset('storage/movies/bg-image/' . $randMovie->bg_url) }}"
                            class="d-block w-100 header-image" alt="...">
                        <div class="carousel-caption d-none d-md-block fw-bolder">
                            <div class="container px-3 carousel-content text-light d-flex flex-column align-items-start">
                                <p class="carousel-sub">
                                    {{ $randMovie->genres->first()->name }} |
                                    {{ $randMovie->release_date->format('Y') }}</p>
                                ...
                                @auth
                                    @if (!auth()->user()->isAdmin())
                                        @can('addWatchList', $randMovie)
                                            <button class="btn btn-danger btn-add-watchlist" value="{{ $randMovie->id }}"><i
                                                    class="fas fa-plus"></i> Add
                                                To
                                                Watchlists</button>
                                        @else
                                            <button class="btn btn-success btn-add-watchlist added"
                                                value="{{ $randMovie->id }}"><i class="fas fa-check"></i> Already
                                                in Watchlist</button>
                                        @endcan
                                    @endif
                                @endauth
    ...
    <section class="main-body">
        ...
        <ul class="movie-carousel card-group list-unstyled cs-hidden text-light" id="autoWidth">
            @foreach ($trendingMovies as $trendMovie)
                <li class="card">
                    <a href="{{ route('show-movie', $trendMovie->id) }}"><img
                            src="{{ asset('storage/movies/thumbnail/' . $trendMovie->image_url) }}"
                            class="card-img-top h-100" alt="..."></a>
                    <div class="card-body">
